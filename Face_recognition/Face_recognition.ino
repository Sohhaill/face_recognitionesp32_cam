...
void handle_message(WebsocketsClient &client, WebsocketsMessage msg)
{
  ...
  if (msg.data() == "start_video_streaming") {
    // Start sending video data
    Serial.println("Hello Flutter");
  }
...
void captureAndSendImage(WebsocketsClient &client) {
  ...
  // Send captured image via HTTP POST
  int httpResponseCode = http.POST((uint8_t*)fb->buf, fb->len);
  
  if (httpResponseCode > 0) {
    Serial.printf("[HTTP] Image sent, status code: %d\n", httpResponseCode);
  } else {
    Serial.printf("[HTTP] Image sending failed, error: %s\n", http.errorToString(httpResponseCode).c_str());
  }
        ...
        if (align_face(out_res.net_boxes, image_matrix, aligned_face) == ESP_OK)
        {
          ...
          if (g_state == START_RECOGNITION  && (st_face_list.count > 0))
          {
            ...
            if (f)
            {
              ...
              sprintf(recognised_message, "Lock OPEN FOR %s", f->id_name);
              open_door(client);
            ...
            else
            {
              ...
              sendHTTPPost("Unauthorized Person detected");
